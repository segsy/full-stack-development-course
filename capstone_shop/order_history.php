...
?>
<!doctype html><html><head><meta charset="utf-8"><title>Your Orders</title><link rel="stylesheet" href="assets/style.css"></head><body>
<div class="container">
  <h2>Your Orders</h2>
  <?php if (empty($orders)): ?><p>No orders yet.</p><?php else: ?>
    <?php foreach($orders as $o): ?>
      <div class="order-card">
        <h3>Order #<?= $o['id'] ?> — <?=htmlspecialchars($o['payment_status'])?> — <?= $o['created_at'] ?></h3>
        <?php
        ...
        <ul>
          <?php foreach($items as $it): ?>
            <li><?=htmlspecialchars($it['title'])?> — <?=intval($it['qty'])?> × <?=money($it['price'])?> = <?=money($it['subtotal'])?></li>
          <?php endforeach; ?>
        </ul>
